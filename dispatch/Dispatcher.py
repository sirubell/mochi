...
if __name__ == "__main__" :
	while(1):
		try:
			# source_url="http://192.168.122.231:5000/dispatcher"
			...
			for i in range (0,Submission_count):
				...
				if Language == "c":
					block = utility.c_block_header("%s/Submission/%s/%s.c"%(file_root,Source_id,Source_id))
					if block == "Block":
						result["Return_Set"].update({Source_id:{"Mode":Mode,"Status":"CE","Compile_error_out":"Use block header","Time":"-1","Memory":"-1","All_stander_out":{},"All_compare_out":{}}})
						continue
				elif Language == "c++":
					block = utility.cpp_block_header("%s/Submission/%s/%s.cpp"%(file_root,Source_id,Source_id))
					if block == "Block":
						result["Return_Set"].update({Source_id:{"Mode":Mode,"Status":"CE","Compile_error_out":"Use block header","Time":"-1","Memory":"-1","All_stander_out":{},"All_compare_out":{}}})
						continue
				elif Language == "python":
					block = utility.python_block_header("%s/Submission/%s/%s.exe"%(file_root,Source_id,Source_id))
					if block == "Block":
						result["Return_Set"].update({Source_id:{"Mode":Mode,"Status":"CE","Compile_error_out":"Use block header","Time":"-1","Memory":"-1","All_stander_out":{},"All_compare_out":{}}})
						continue
			...
			# return_url="http://192.168.122.231:5000/dispatcher"
